...
export class Game {
    ...
    private callbacks: Record<string, { update: UpdateCallback; gameOver: GameOverCallback }> = {};
    ...
    start() {
        ...
        this.timer = setInterval(() => this.updateGameState(), 1000 / UPDATES_PER_SECOND);
    }

    join(id: string, name: string, updateCallback: UpdateCallback, gameOverCallback: GameOverCallback) {
        if (!this.timer) {
            this.start();
        }
    ...
    drop(id: string) {
        ...
        this._gameState.players = this._gameState.players.filter(p => p.id !== id);
    ...
    updateGameState() {
        ...
        Object.keys(this.callbacks).forEach(id => {
            ...
            if (!player) {
                ...
                const conquerer = this._gameState.players.find(p => p.id === conquererId);
                const message = conquerer ? `You were eliminated by ${conquerer.name}` : `You were eliminated`;
